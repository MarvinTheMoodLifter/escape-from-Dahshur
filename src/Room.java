...
public class Main_Character extends Entity
{
    ...
    public void equipInvItem(String itemName)
    {
        Item item = inventory.get(itemName);
        if (item != null && !hasEquippedItem)
        {
            equipItem(item);
            inventory.remove(item.getName());
        }
        ...
        {
            System.out.println("Item not found in inventory or already holding an item.");
        }
    }
    ...
    public void unequipItem(Room room)
    {
        if (hasEquippedItem && equippedItem != null)
        {
            ...
            {
                inventory.put(equippedItem.getName(), equippedItem);
