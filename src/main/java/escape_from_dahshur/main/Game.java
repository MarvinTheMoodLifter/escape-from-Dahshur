...
public class Game
{
    ...
    public static void main(String[] args)
    {
        ...
        while (true)
        {
            ...
            if (!inCombat)
            {
                ...
                if (currentRoom.hasItem()) { printCentered("inspect [item_name]", ANSI_GREEN); }
            ...
            else if (input.equalsIgnoreCase("view inventory"))
            {
                ...
                } else if (subInput.equalsIgnoreCase("unequip item")) { hero.unequipItem(pyramid.getRoom(hero.getCurrentPosition()[1], hero.getCurrentPosition()[0])); }
                else if (subInput.toLowerCase().startsWith("drop "))
                {
                    String itemName = subInput.substring(5).trim();
                    if (hero.getInvItem(itemName) != null) { hero.dropItem(hero.getInvItem(itemName), pyramid.getRoom(hero.getCurrentPosition()[1], hero.getCurrentPosition()[0])); }
                    else { System.out.println("you do not have this item"); }
                }
                else if (subInput.toLowerCase().startsWith("check "))
                {
                    String itemName = subInput.substring(6).trim();
                    if (hero.getInvItem(itemName) != null) { hero.getInvItem(itemName).printItemDesc(); }
                    else { System.out.println("you do not have such an item"); }
                }
            ...
            else if (input.toLowerCase().startsWith("talk to ") && !inCombat)
            {
                ...
                if (npc != null)
                {
                    ...
                    if (hero.getCurrentPosition()[1] == 2 && hero.getCurrentPosition()[0] == 0 && pyramid.getRoom(2, 0).findNPCByName("lost explorer").isAlive()) { printCentered("free lost explorer ", ANSI_GREEN); }
                    printCentered("back", ANSI_BLUE);
                    System.out.print("Enter action: ");
                    String subInput = scanner.nextLine();
                    ...
                    if (subInput.toLowerCase().startsWith("attack "))
                    {
                        ...
                        if (attackNPCName.equalsIgnoreCase(npcName))
                        {
                            ...
                            if (npc != null && npc.isAlive())
                            {
                                ...
                                if(!npc.isAlive()){ hero.setScore(hero.getScore()+npc.getScore()); }
                                if (npc.isAlive())
                                {
                    ...
                    else if (subInput.toLowerCase().startsWith("free ") && (hero.getCurrentPosition()[1] == 2 && hero.getCurrentPosition()[0] == 0) && pyramid.getRoom(2, 0).findNPCByName("lost explorer").isAlive())
                    {
                        ...
                        if (interactednpcname.equalsIgnoreCase(npcName))
                        {
                            ...
                            if (npc != null && npc.isAlive()) { npc.NpcInteraction(interactednpcname, "free", hero, pyramid.getRoom(currentPosition[1], currentPosition[0])); }
                            else if (npc != null && !npc.isAlive()) { printCentered(npc.getName() + " is already dead and cannot be interacted with."); }
                            else { printCentered("No NPC named '" + interactednpcname +  "' found in the room."); }
                        }
                        else { printCentered("NPC name does not match the talked NPC."); }
                    }
                    else { printCentered("Unknown command. Returning to main menu."); }
                }
                else { printCentered("No NPC named '" + npcName + "' found in the room."); }
            }
            ...
            else if (input.toLowerCase().startsWith("leave pyramid") && hero.getCurrentPosition()[1] == 0 && hero.getCurrentPosition()[0] == 0 && !inCombat)
            {
                ...
                else { System.out.println("you did not equip any item."); }
            }
    ...
    protected static void saveGame(Main_Character hero, Pyramid pyramid, String saveName)
    {
        ...
        catch (IOException e) { printCentered("Error saving game: " + e.getMessage(), ANSI_RED); }
    }

    protected static void loadGame(Main_Character hero, Pyramid pyramid, String saveName)
    {
        ...
        catch (IOException e) { printCentered("Error loading game: " + e.getMessage(), ANSI_RED); }
    }
    ...
    {
        if (hero.getScore()+hero.getInvScore() <= 0) { System.out.println("you did not bring back anything noteworthy.\n" + "this entire expedtion was a waste of time....at least you didn't die"); }
        else
        {
